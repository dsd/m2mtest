	...
	int *consumed, int *frame_size, char get_head)
{
	...
	*consumed = 0;
	...
	while (in_size-- > 0) {
		...
		if (got_start == 0 && ctx.headers_count == 1 && ctx.main_count == 0) {
			ctx.code_start = ctx.tmp_code_start;
			got_start = 1;
		}

		if (got_start == 0 && ctx.headers_count == 0 && ctx.main_count == 1) {
			ctx.code_start = ctx.tmp_code_start;
			got_start = 1;
			seek_end = 1;
			ctx.headers_count = 0;
		...
		if (seek_end == 0 && ctx.headers_count > 0 && ctx.main_count == 1) {
			seek_end = 1;
			ctx.headers_count = 0;
			ctx.main_count = 0;
		}

		if (seek_end == 1 && (ctx.headers_count > 0 || ctx.main_count > 0)) {
			ctx.code_end = ctx.tmp_code_start;
			ctx.got_end = 1;
